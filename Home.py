...
def modulo_ventas():
        ...
        with st.form("form_nuevo_cliente"):
            ...
            if submit_nuevo_cliente:
                if nombre_cliente.strip() == "":
                    ...
                    # Guardar en Excel
                    try:
                        with pd.ExcelWriter('AdministracionSoop.xlsx', engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
    ...
    if st.session_state.get('mostrar_formulario_editar_cliente', False):
        ...
        cliente_df = st.session_state.df_clientes[st.session_state.df_clientes['Nombre'] == cliente_seleccionado]
        if cliente_df.empty:
    ...
    if cliente_seleccionado != "" and not st.session_state.get('mostrar_formulario_cliente', False) and not st.session_state.get('mostrar_formulario_editar_cliente', False):
        ...
        if cliente_df.empty:
                ...
                with st.form("form_editar_notas"):
                    ...
                    if submit_nuevas_notas:
                        ...
                        try:
                            ...
                            st.error(f"Error al actualizar las notas: {e}")
            ...
            vendedor_asignado_value = cliente_data.get('Vendedores', vendedores[0])
            if pd.isna(vendedor_asignado_value) or vendedor_asignado_value not in vendedores:
                vendedor_index = 0
            else:
