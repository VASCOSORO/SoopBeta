...
def modulo_ventas():
    ...
    if cliente_seleccionado != "":
        ...
        # Mostrar descuento arriba de vendedor asignado y al lado del título
        col_desc_vend, col_desc_vend_2 = st.columns([1, 1])

        with col_desc_vend:
            st.write(f"**Descuento:** {cliente_data['Descuento']}%")
        
        with col_desc_vend_2:
        ...
        # Mostrar última compra, estado de crédito y forma de pago
        col_compra, col_credito, col_pago = st.columns([1, 1, 1])

        with col_compra:
            st.write(f"**Última compra:** {cliente_data['Fecha Modificado']}")
        
        with col_credito:
            st.write(f"**Estado de crédito:** 🟡 Pagos regulares")
        
        with col_pago:
            st.selectbox("Forma de Pago", ["Contado", "CC", "Depósito/Transferencia"])
        
        # Filtrar por rubro del cliente
        rubros_cliente = cliente_data['Rubros'].split(',') if pd.notna(cliente_data['Rubros']) else []
        if rubros_cliente:
            rubro_seleccionado = st.selectbox("📝 Filtrar por Rubro del Cliente", [""] + rubros_cliente)
        else:
            rubro_seleccionado = None

        # Mostrar productos filtrados
        if rubro_seleccionado:
            productos_filtrados = st.session_state.df_productos[st.session_state.df_productos['Rubro'] == rubro_seleccionado]
            st.write(f"Mostrando {len(productos_filtrados)} productos disponibles para el rubro '{rubro_seleccionado}'.")
        else:
            productos_filtrados = st.session_state.df_productos
            st.write(f"Mostrando todos los productos disponibles.")
        ...
        if producto_buscado:
            ...
            # Código del producto, imagen y cantidad
            col_izq, col_der = st.columns([2, 1])

            with col_izq:
                st.write(f"**Código del producto:** {producto_data['Codigo']}")
                cantidad = st.number_input("Cantidad", min_value=1, max_value=stock, step=1, key=f"cantidad_{producto_data['Codigo']}")
        ...
        if st.session_state.pedido:
            ...
            for producto in st.session_state.pedido.copy():
                ...
                if producto['Codigo'] in st.session_state.delete_confirm:
                    with col6:
                        if st.button("Sí", key=f"confirmar_si_{producto['Codigo']}"):
                            ...
                            del st.session_state.delete_confirm[producto['Codigo']]
                else:
                    with col6:
                        if st.button('🗑️', key=f"eliminar_{producto['Codigo']}"):
                            st.session_state.delete_confirm[producto['Codigo']] = True
            ...
            col_items.write(f"**Total de ítems:** {total_items}")
            col_total.write(f"<h4 style='text-align:right;'>Total del pedido: ${total_monto:,.2f}</h4>", unsafe_allow_html=True)
